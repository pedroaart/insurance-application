...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handlePolicyAlreadyExistsException(PolicyAlreadyExistsException ex) {
        log.error("Policy already exists: {}", ex.getMessage());
    ...
    public ResponseEntity<ErrorResponse> handleDuplicateRequestException(DuplicateRequestException ex) {
        log.error("Duplicate request: {}", ex.getMessage());
        
        ErrorResponse error = ErrorResponse.builder()
            .timestamp(LocalDateTime.now())
            .status(HttpStatus.CONFLICT.value())
            .error("Duplicate Request")
            .message(ex.getMessage() + " (Idempotency Key: " + ex.getIdempotencyKey() + ")")
            .build();
    ...
    public ResponseEntity<ErrorResponse> handleCallNotPermittedException(CallNotPermittedException ex) {
        ...
        ErrorResponse error = ErrorResponse.builder()
            ...
            .message("The customer validation service is temporarily unavailable. Please try again in a few moments.")
            .build();
